...
<div class="py-4 max-w-xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-lg font-semibold text-gray-900">Editar Setor: {{ $setor->nome }}</h2>
    </div>
            ...
            <div>
                ...
                <select name="empresa_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    @foreach($empresas ?? [] as $empresa)
                        <option value="{{ $empresa->id }}" {{ old('empresa_id', $setor->empresa_id) == $empresa->id ? 'selected' : '' }}>{{ $empresa->nome }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <textarea name="descricao" rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('descricao', $setor->descricao) }}</textarea>
            </div>
            <label class="flex items-center gap-2 text-sm text-gray-700">
                <input type="hidden" name="ativo" value="0">
                <input type="checkbox" name="ativo" value="1" {{ old('ativo', $setor->ativo) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600">
                Setor ativo
            </label>
        </div>
        <div class="flex items-center justify-end gap-3 mt-4">
            <a href="{{ route('setores.index') }}" class="px-5 py-2.5 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 text-sm font-medium rounded-lg transition">Cancelar</a>
