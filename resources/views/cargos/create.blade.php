...
<div class="py-4">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-lg font-semibold text-gray-900">Cadastrar Novo Cargo</h2>
    </div>
    ...
    <form method="POST" action="{{ route('cargos.store') }}">
            ...
            <div>
                ...
                <select name="nivel_hierarquico_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    @foreach($niveisHierarquicos ?? [] as $nivel)
                        <option value="{{ $nivel->id }}" {{ old('nivel_hierarquico_id') == $nivel->id ? 'selected' : '' }}>{{ $nivel->nome }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Salário Base (R$)</label>
                <input type="number" name="salario_base" data-money value="{{ old('salario_base') }}" step="0.01" min="0"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Salário Máximo (R$)</label>
                <input type="number" name="salario_maximo" data-money value="{{ old('salario_maximo') }}" step="0.01" min="0"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>
            <div>
        ...
        <div class="flex items-center justify-end gap-3 mt-4">
            ...
            <button type="submit" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg shadow-sm transition">Salvar Cargo</button>
        </div>
    </form>
</div>
@endsection
